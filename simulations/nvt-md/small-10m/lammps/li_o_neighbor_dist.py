...
def main():
    ...
    for (temp), group in project.groupby(("T")): 
        ...
        for job in group:
            ...
            full_unwrap_traj = md.load(job.fn("unwrap_mol.lammpstrj"), top=top)
            ...
            if full_traj.n_frames>10000:
                ...
                traj_list.append(traj[10000:])

                unwrap_traj = md.Trajectory(
                        full_unwrap_traj.xyz,
                        full_unwrap_traj.top,
                        unitcell_lengths = np.tile([length[0]/10, length[1]/10, length[2]/10], (full_unwrap_traj.n_frames,1)),
                        unitcell_angles = np.tile([90.,90.,90.], (full_unwrap_traj.n_frames,1)),
                    )
                unwrap_traj_list.append(unwrap_traj[10000:])

                traj_lengths.append(full_traj.n_frames-10000)
