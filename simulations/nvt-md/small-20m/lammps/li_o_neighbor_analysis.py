...
def main():
    ...
    for (temp), group in project.groupby(("T")):
        ...
        #get the sh rtest traj and reduce all trajs to that length
        print("All trajs loaded")
        box = freud.box.Box(Lx=length[0]/10,Ly= length[1]/10, Lz=length[2]/10)
        ...
        for traj in traj_list:
            ...
            while start_frame<(end_length-chunk_length):
                ...
                chunk.time = np.linspace(0*1000/1000, (chunk_length-1), len(chunk.time))
                dt = get_dt(chunk)
        ...
        t = np.array(list(range(len(fraction_overall))))
