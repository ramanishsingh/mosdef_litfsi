...
def main():
    ...
    for (temp, functional), group in project.groupby(("T","Functional")):
        ...
        for job in group:
            ...
            i=0
            count = 0 #this ocunt to weight the trajectories

            while (i*skip_steps+window_width_steps) < len(wat_O_xyz):
                print(i*skip_steps+window_width_steps)
                window_xyz = wat_O_xyz[i*skip_steps:(i*skip_steps+window_width_steps)]
            ...
            fig.savefig("loglog_msd_{}.pdf".format(job))

            plt.close()

            msd_list.append(msdA2)
            weights.append(count)
            prod_lengths.append((one_traj.n_frames)*0.005)
            job_names.append(small_jobname)        
        ...
        for i, msd in enumerate(msd_list):
            plt.plot(time, msd, label = "job = {}, length = {}".format(job_names[i], prod_lengths[i]))

        plt.axvline(x=start,color='red',alpha=0.3, ls='--')
        plt.axvline(x=end,color='red',alpha=0.3, ls='--')
        ax.set_xlabel('Time /ps')
        ax.set_ylabel(r"MSD / $\mathrm{\AA}^2$")
        plt.plot(time, average_msdA2, label = "Average")
        plt.legend()

        fig.savefig("overall_msd.pdf".format(job))
        plt.close()
